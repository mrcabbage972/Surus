...
#' @details Robust Principal Component Pursuit is a matrix decomposition algorithm that seeks to separate a matrix X into the sum of three parts X = L + S + E. L is a low rank matrix representing a smooth X, S is a sparse matrix containing corrupted data, and E is noise. To convert a time series into the matrix X we take advantage of seasonality so that each column represents one full period, for
...
AnomalyDetection.rpca = function(X, frequency=7, dates=NULL, autodiff = T, forcediff = F, scale = T, L.penalty = 1, s.penalty=1.4 / sqrt(max(frequency, ifelse(is.data.frame(X), nrow(X), length(X)) / frequency)), verbose=F) {
  ...
  rpca.ts = apply(X, 2, function(j) {
    ...
    list(rpca = RcppRPCA(j.matrix, Lpenalty = L.penalty, Spenalty = s.penalty, verbose=verbose), mean = j.global.mean, sd = j.global.sd, diff = useddiff, j.init = j.init)
  })
  rpca.ts.stacked = lapply(rpca.ts, function(i) {
    if (i$diff) {
      ...
      data.frame(X.transform = X.transform, L.transform = L.transform, S.transform = S.transform, E.transform = E.transform, X.orig = X.orig, time = time)[-1,]
    }
    else {
      data.frame(X.transform = (as.vector(i$rpca$X)) * i$sd + i$mean, L.transform = (as.vector(i$rpca$L)) * i$sd + i$mean, S.transform = (as.vector(i$rpca$S)) * i$sd, E.transform = (as.vector(i$rpca$E)) * i$sd, X.orig = (as.vector(i$rpca$X)) * i$sd + i$mean, time = time)
    }
  })
  ...
  names(rpca.ts.stacked) = c("X_transform", "L_transform", "S_transform", "E_transform", "X_original", "time", "name")
